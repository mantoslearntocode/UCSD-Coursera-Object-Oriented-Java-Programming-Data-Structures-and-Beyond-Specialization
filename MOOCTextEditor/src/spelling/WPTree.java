...
//		}
		...
		int count = 0;
		Queue<WPTreeNode> myNodeList = new LinkedList<WPTreeNode>();
		...
		while (myNodeList != null && !myNodeList.isEmpty() && !find && count < 1000)
		{
			WPTreeNode curNode = myNodeList.remove();
			List<String> mySugs = nw.distanceOne(curNode.getWord(), true);
			for (String mySug : mySugs)
			{
				if (count >= 1000)
				{
					return null;
				}
				if (!visited.contains(mySug))
				{
					count++;
					WPTreeNode newNode = curNode.addChild(mySug);
					visited.add(mySug);
					myNodeList.add(newNode);
//					System.out.println(newNode.buildPathToRoot());
					if (mySug.equals(word2))
					{
						find = true;
